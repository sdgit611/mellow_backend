    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('sku', function ($row) {
                return $row->product && !is_null($row->product->sku) ? $row->product->sku : '----';
            })
            ->editColumn('net_quantity', function ($row) {
                    return $row->net_quantity ? $row->net_quantity : '----';
            })
            ->editColumn('changed_value', function ($row) {
                return currency_format($row->changed_value, company()->currency->id);
            })
