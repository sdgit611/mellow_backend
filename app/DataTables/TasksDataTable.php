...
class TasksDataTable extends BaseDataTable
{
    ...
    {
        return $model->newQuery()->whereHas('applicationStatus', function ($query) {
    ...
    public function dataTable($query)
    {
        ...
        ->eloquent($query)
        ->addColumn('title','--')
        ->addColumn('heading', fn($row) => $row->full_name)
        ->addColumn('short_code', fn($row) => $row->email)
        ->addColumn('completed_on', fn($row) => $row->phone)
        ->addColumn('start_date', fn($row) => $row->start_date ?? '-')
        ->addColumn('due_date', fn($row) => $row->due_date ?? '-')
        ->addColumn('estimateTime', fn($row) => $row->created_at?->format('d M Y') ?? '-')
        ->addColumn('timeLogged', fn($row) => $row->updated_at?->format('d M Y') ?? '-')

        ->addColumn('assignedTo', fn($row) => $row->assigned_to ?? '-')
        ->addColumn('board_column', fn($row) => optional($row->applicationStatus)->status ?? 'N/A')
        ->addColumn('title', function ($row) {
            // return '<a href="' . route('recruit-applications.show', $row->id) . '" class="btn btn-sm btn-primary">View</a>';
...
//            $data[__('app.timer') . ' '] = ['data' => 'timer', 'name' => 'timer', 'exportable' => false, 'searchable' => false, 'sortable' => false, 'title' => __('app.timer'), 'class' => 'text-right'];
        ...
        $data[__('app.task') . ' ' . __('app.status')] = ['data' => 'status', 'name' => 'board_column_id', 'visible' => true, 'title' => __('app.task') . ' ' . __('app.status')];
